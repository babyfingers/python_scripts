...
for i in range(npts):
    ...
    f.write(template.replace(geo_m,geo_sp).replace(scf_m,rds).replace(cis_m,"true").replace(tia_m,tia_s).replace(fn_m,"x = "+str(i)+"/"+str(npts-1)+", +dx geometry."))
    f.close()
    f = open(mpfx+infn_l[i],ws)
    f.write(template.replace(geo_m,geo_sm).replace(scf_m,rds).replace(cis_m,"true").replace(tia_m,tia_s).replace(fn_m,"x = "+str(i)+"/"+str(npts-1)+", -dx geometry."))
    f.close()
    if (tia_i>1):
        ...
        f.write(template.replace(geo_m,geo_s0+eols+geo_sp).replace(scf_m,rds).replace(cis_m,"true").replace(tia_m,tia_sS).replace(fn_m,"Cross-geometry AO overlap calculation for x = "+str(i)+"/"+str(npts-1)+", +dx geometry."))
        f.close()
        f = open(Ss+mpfx+infn_l[i],ws)
        f.write(template.replace(geo_m,geo_s0+eols+geo_sm).replace(scf_m,rds).replace(cis_m,"true").replace(tia_m,tia_sS).replace(fn_m,"Cross-geometry AO overlap calculation for x = "+str(i)+"/"+str(npts-1)+", -dx geometry."))
        f.close()
    else:
