...
# previous calculations. (Hence the special Q-Chem script, msqchem.csh.

#User-adjusted variables
hh       = 0.000000001 #Incremental h value
ha       = 0.000000001 #Starting h value
hb       = 0.000000009 #Ending h value
        ...
        if (goh==1):
        	#Write all of the files necessary to calculate the gradient
                for hi in narray:
                    ...
                    fp.write(template_s.replace(li,lip).replace(m1,rds).replace(m2,ons))
                    fm.write(template_s.replace(li,lim).replace(m1,rds).replace(m2,ons))
                    Sfp.write(Stemplate_s.replace(li,lip).replace(m1,rds).replace(m2,ons))
                    Sfm.write(Stemplate_s.replace(li,lim).replace(m1,rds).replace(m2,ons))
...
if (mode1==2 or mode1==3):
    ...
    Sqmk_r = re.compile(uns + filenamebase + outfilenameend + ".* [RQ] batch")
    ...
    for ifl in range(jobstart,nfiles):
        if (ifl>0):
            time.sleep(1.0)
            print "Waiting " + str(1.0) + " seconds."
            #Part one: run the overlap file
            os.system(qstat)
            f = open(tmpfl,rs)
            fs = f.read()
            f.close()
            Snrunning = len(re.findall(qmk_r,fs))
            while (Snrunning >= maxj):
            ...
            fs = f.read()
            f.close()
            nrunning = len(re.findall(Sqmk_r,fs))
            while (nrunning >= maxj):
                print "Waiting " + str(tm) + " seconds."
                time.sleep(tm)
                os.system(qstat)
...
if (mode1==2 or mode1==3 or mode1==4):
    ...
    os.system("grep xxcMOxOvr_stdaa1xx *"+filenamebase+outfilenameend+" > cMOxOvr_stdaa1.txt")
    os.system("grep xxcMOxOvr_stdaa2xx *"+filenamebase+outfilenameend+" > cMOxOvr_stdaa2.txt")
    os.system("grep xxcMOxOvr_stdab1xx *"+filenamebase+outfilenameend+" > cMOxOvr_stdab1.txt")
    os.system("grep xxcMOxOvr_stdab2xx *"+filenamebase+outfilenameend+" > cMOxOvr_stdab2.txt")
    os.system("grep xxMOxOvr_astdxx *"+filenamebase+outfilenameend+" > MOxOvr_astd.txt")
    os.system("grep xxMOxOvr_aNewxx *"+filenamebase+outfilenameend+" > MOxOvr_aNew.txt")
    print "Now analyzing the output."
    quant = ["MOxOvr_std","cMOxOvr_stdaa1","cMOxOvr_stdaa2","MOxOvr_astd","MOxOvr_aNew"]
    quantfn = ["MOxOvr_std.txt","cMOxOvr_stdaa1.txt","cMOxOvr_stdaa2.txt","MOxOvr_astd.txt","MOxOvr_aNew.txt"]
    numq = len(quant)
    ...
    if (goh==1):
		...
		m_raw = re.findall(m_re,fq_s)
                ...
                for I in range(numkeep):
                    ...
                    l_p = len(p_ar)
                    l_m = len(m_ar)
                    #Catch and discrepancy between the number of values obtained and what we expect.
                    if (l_p != l_m):
