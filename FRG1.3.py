...
# v1.3: modified some more to account for new use of tia_order: made
# it so that the "S" jobs are only run if full overlap calculations
# are specified.
...
dtia = ttovlp
ttia = str(dtia) #The type of overlap calculation to perform
meps = 'eeppss'
seps = str(veps)
mTmp = 'TTmmpp'
sTmp = str(vTmp)
...
for i in narray:
    if (dtia==2):
        ...
        ii = i
    infile.append(filenamebase+szz1[ii]+infilenameend)
    outfile.append(szz1[ii]+filenamebase+outfilenameend)
    print "Writing "+infile[ii]
    f_i = open(infile[ii],'w')
    f_i.write(template_s.replace(mzz1,szz1[ii]).replace(msgf,ssg[i]).replace(mtia,stia[i]).replace(meps,seps).replace(mTmp,sTmp))
    f_i.close()
    if (i < nfiles-1 & dtia==2):
        infile.append(sS+filenamebase+szz1[ii+1]+infilenameend)
        outfile.append(sS+szz1[ii+1]+filenamebase+outfilenameend)
        print "Writing "+infile[ii+1]
        f_i = open(infile[ii+1],'w')
        f_i.write(Stemplate_s.replace(mzz1,szz1[ii+1]).replace(mzz2,szz1[ii+2]))
        f_i.close()                
template.close()
if (dtia==2):
    Stemplate.close()
...
if Mmkrun==2:
    ...
    for i in range(read_job_start,len(szz1)):
        if (Mstab==1 and Mscfg==1):
