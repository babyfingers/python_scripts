...
#              of any size-- look out for that in Version 1.7.
...
for i in range(npts):
    ...
    if (mode1==1 or mode1==3):
        ...
        f.write(template.replace(geo_m,geo_sp).replace(scf_m,rds).replace(cis_m,cis_st).replace(tia_m,tia_s).replace(fn_m,"x = "+str(i)+"/"+str(npts-1)+", +dx geometry. h = "+str(hh)))
        f.close()
        f = open(mpfx+infn_l[i],ws)
        f.write(template.replace(geo_m,geo_sm).replace(scf_m,rds).replace(cis_m,cis_st).replace(tia_m,tia_s).replace(fn_m,"x = "+str(i)+"/"+str(npts-1)+", -dx geometry. h = "+str(hh)))
        f.close()
    if (tia_i>1):
        ...
        if (mode1==1 or mode1==3):
            ...
            f.write(template.replace(geo_m,geo_s0+eols+geo_sp).replace(scf_m,rds).replace(cis_m,cis_st).replace(tia_m,tia_sS).replace(fn_m,"Cross-geometry AO overlap calculation for x = "+str(i)+"/"+str(npts-1)+", +dx geometry. h = "+str(hh)))
            f.close()
            f = open(Ss+mpfx+infn_l[i],ws)
            f.write(template.replace(geo_m,geo_s0+eols+geo_sm).replace(scf_m,rds).replace(cis_m,cis_st).replace(tia_m,tia_sS).replace(fn_m,"Cross-geometry AO overlap calculation for x = "+str(i)+"/"+str(npts-1)+", -dx geometry. h = "+str(hh)))
            f.close()
    else:
