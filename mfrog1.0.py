...
pta  = 10.00  #Start the value where?
ptb  = 1.2  #End the value where?
inc  = -0.05
...
if (mode2==1):
    mode3 = input("Input SCF guess from previous calculation (1) or from individual set of SCF guess folders (2)?\n")
else:
    mode3 = 1
...
for i in narray:
    ...
    f_i = open(infile[i],'w')
    f_i.write(template_s.replace(m1,s1[i]).replace(m2,s2[i]))
    f_i.close()
template.close()
...
if mode1==2:
    ...
    if (mode4==1):
        ...
        if (mode2==1 and mode3==1):
            sv = " -outdir scf_read -save standard"
        elif (mode2==2):
            sv = " -outdir scf_read"+s1[0]+" -save standard"
        elif (mode3==2):
            sv = " -indir scf_read"+s1[0]
        command = "myqchem.csh -in "+infile[0]+" -out "+outfile+sv
    ...
    for i in range(read_job_start,nfiles):
        jobs_running = 1 #When this drops below 1, the job has completed
        if (mode2==1 and mode3==1):
            sv = " -indir scf_read -outdir scf_read -save standard"
        elif (mode2==2):
            sv = " -indir scf_read"+s1[i-1]+" -outdir scf_read"+s1[i]+" -save standard"
        elif (mode3==2):
