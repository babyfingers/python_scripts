...
# TODO: right now, if we DON'T sum over the operator indices (e.g. p, q)
# but do sum over state indices (e.g. ijab), then any time we encounter
# a delta function connecting one to the other, we still obtain a final
# summation at the end. This ought to be corrected so that the summation
# is truncated to the equality, e.g. \sum_{i=p, j=q}.
# TODO: Does the code assume that the one-electron operator is symmetric?
# YES. Make a new one that doesn't.
...
# v1.5: enabled the option of forgoing an operator,
#       op_mode = 4.

# v1.4: fixed a bug that had to do with proper ordering of
#       the second quantization operators. Also, note that
#       this version, like all previous versions, only works
#       if each state and operator has an equal number of
#       creation/annihilation operators (see lt_to_sq).
#       Also, just generally made the indexing/ordering
#       process easier to follow (I hope) if you learned
#       second quantization from Szabo & Ostlund. I got rid
#       of the stuff that had to do with putting the 2e
#       operator in physicist's notation until the very end,
#       that didn't make any sense to me.

# v1.3: Added support for Hartree-Fock reference states.

# v1.2: Changed the two-electron operator Pi to print as the
#       two-electron integral that appears in the electronic
#       Hamiltonian. Assumptions made to collect like terms
#       include symmetries based on these integrals, and
#       real orbitals are also assumed.
...
op_re = re.compile("([A-z]+_\{(?:\<[A-z][0-9]?\>)+\})(\Z)?(?(2)|[^\^])")
#op_re  = re.compile("([A-z]+_\{(?:\<[A-z][0-9]?\>)+\})[^\^]?")
#opind_re  = re.compile("\{[infx]+_\{((?:\<[A-z][0-9]?\>)+)\}[tg]")
optind_re = re.compile("[A-z]+_\{((?:\<[A-z][0-9]?\>)+)\}(\Z)?(?(2)|[^\^])")
oprind_re = re.compile("[A-z]+_\{((?:\([A-z][0-9]?\))+)\}(\Z)?(?(2)|[^\^])")
#optind_re = re.compile("[A-z]+_\{((?:\<[A-z][0-9]?\>)+)\}[^\^]?")
#oprind_re = re.compile("[A-z]+_\{((?:\([A-z][0-9]?\))+)\}[^\^]?")
...
#          Remember: for appropriate ordering of operators, we
#          must reverse the ordering of the annihilation operators
#          from the way that they are presented in the LaTeX-
#          formatted string.
#          NOTE: I guess this function assumes
#          that all states/operators have an equal number of
#          creation/annihilation indices.
def lt_to_sq(lt_s):
    s = ""
    op_lst  = re.findall(op_re ,lt_s)
    if (len(op_lst)>0):
        op_lst = op_lst[0]
        #print "lt_to_sq: op_lst:",op_lst
        #print "lt_to_sq: lt_s:",lt_s
    tia_lst = re.findall(tia_re,lt_s)
    gia_lst = re.findall(gia_re,lt_s)
    #Left side, first excitations
    if (lstate_mode != 5 and len(tia_lst)>0):
        temp_lst = re.findall(tri_re,tia_lst[0])
        hnt = len(temp_lst)/2
        crt_lst = list(temp_lst[0:hnt])
        anh_lst = list(temp_lst[hnt:2*hnt])
        anh_lst.reverse()
        for i in range(hnt):
            s += "("+crt_lst[i]+"+)"
        for i in range(hnt):
    ...
    if (lstate_mode == 4 and len(gia_lst)>0):
    ...
    if (rstate_mode == 4 and len(gia_lst)>0):
        if (lstate_mode == 4):
            temp_lst = re.findall(tri_re,gia_lst[1])
        else:
            temp_lst = re.findall(tri_re,gia_lst[0])
        hnt = len(temp_lst)/2
        anh_lst = list(temp_lst[0:hnt])
        crt_lst = list(temp_lst[hnt:2*hnt])
        anh_lst.reverse()
        for i in range(hnt):
            s += "("+crt_lst[i]+"+)"
        for i in range(hnt):
            s += "("+anh_lst[i]+"-)"                            
    #Right side, first excitations
    if (rstate_mode != 5 and len(tia_lst)>0):
        if (lstate_mode != 5):
...
lstate_mode = input("Left state : singles (1), doubles (2), triples (3), singles on singles (4), Hartree-Fock ground state (5), or custom (0)?\n")
#print "You have chosen: ",lstate_mode
rstate_mode = input("Right state: singles (1), doubles (2), triples (3), singles on singles (4), Hartree-Fock ground state (5), or custom (0)?\n")
#print "You have chosen: ",rstate_mode
print "What kind of operator would you like to use?"
op_mode = input("Symmetric one-electron operator X (1), two-electron operator Pi (2), Fock matrix F (3), non-symmetric one-electron operator (4), no operator (5), or custom (0)?\n")
...
if (op_mode==1):
    #Arbitrary SYMMETRIC one-electron operator
    op_s = "X_{<r><s>}"
    oop_s = "X"
    key_lst.append("r")
    key_lst.append("s")
    ...
    #Non-symmetric one-electron operator
    op_s = "Y_{<r><s>}"
    oop_s = "Y"
    key_lst.append("r")
    key_lst.append("s")
...
elif (lstate_mode>3 and lstate_mode<5):
...
elif (rstate_mode>3 and rstate_mode<5):
...
print "Full latex template expression:"
print lt_tmp
...
#      operators. Don't bother, except for primary term
#      summation symbol (shouldn't contain wild card
#      indices).
...
#        Key: t    = CI amplitude (single, double, etc.)
#             g    = further excitation (single only)
...
# outterm: Same as above, but for out_lst terms.
def outterm(l):
    l2 = []
    for t in l:
        l2.append([0,[]])
    ...
    if (ca_count(l)==2):
        ...
        if (not orb_type(o0)) and (not orb_type(o1)):
            if (is_cre(o0) and not is_cre(o1)):
                ...
                (l[0])[2] = []
                #print "cleanup: wild card delta function created"
    elif (ca_count(l)==4):
        o0 = str(((l[0])[2])[0])
        o1 = str(((l[0])[2])[1])
        o2 = str(((l[0])[2])[2])
...
def sq_switch(l):
    #Don't bother if there are no creation/
    #annihilation terms.
    if len((l[0])[2])==0:
        return l
    n = choose_move(l)
...
#           rstu -> tsru symmetric, and rstu -> srut
#           symmetric symmetry operations allowed.
def opindord(t):
    #print "opindord: t (before):",t
    temp_lst = re.findall(optind_re,(t[0])[2])
    temp_lst = temp_lst[0]
    ...
    if (len(tind_lst)==2): #One-electron operator
        ...
        temp_lst = [ind1,ind0]
        perm_lst.append([temp_lst,fstrtoint(temp_lst),1])
    if (len(tind_lst)==4): #Two-electron operator, <rs|tu>
        ind0 = tind_lst[0]
        ind1 = tind_lst[1]
        ind2 = tind_lst[2]
        ind3 = tind_lst[3]
        ...
        perm_lst.append([temp_lst,fstrtoint(temp_lst),1])
    #print "opindord: perm_lst:",perm_lst
...
# nozero: Takes a standard formatted output list, removes
#         terms with coefficient of zero.
def nozero(l):
    for ti in range(len(l)-1,-1,-1):
        if ((l[ti])[0]==0):
            #print "Pop goes the zero term."
            l.pop(ti)
    return l

# printl: Takes a list and string, print each element of
#         the list on a new line, with the string in
#         front of it.
def printl(s,l):
    for t in l:
        print s+": ",t
...
#only instead). Then standardize the index order and replace
#any remaining round brackets with triangular ones.
...
#printl("out_lst (1)",out_lst)

#Standardize output 2: standardize the order of the operator
#indices. For this, symmetric one-electron operators X_{rs} are assumed
#to be symmetric under the transformation rs -> sr, and two-
#electron operators Pi_{rstu}=<rs|tu> are assumed to be symmetric
#under the transformation rstu -> tsru and under the
#transformation rstu -> turs.
if (op_mode==0 or op_mode==1 or op_mode==2 or op_mode==3):
    for ti in range(len(out_lst)-1,-1,-1):
        out_lst += opindord([out_lst[ti]])
        out_lst.pop(ti)
    out_lst.reverse()
    #printl("out_lst (2)",out_lst)

#Standardize output 3: Check for matching terms. If they are found, put them together
#by deleting one and adding its numerical coefficients to the other.
for ti in range(len(out_lst)-1,0,-1):
    for cti in range(ti-1,-1,-1):
        if ((out_lst[ti])[1]==(out_lst[cti])[1] and (out_lst[ti])[2]==(out_lst[cti])[2]):
            ...
            (out_lst[ti])[0] = 0
            break
out_lst = nozero(out_lst)
#printl("out_lst (3)",out_lst)

# strmatch4: Takes two LaTeX-formatted strings, each representing the
#            output for a single term. Return true if the two terms
#            can be combined into a Coulomb-exchange integral, e.g.
#            <rs||tu> = <rs|tu>-<rs|ut>.
#            String example:
#             't_{<i><j>}^{I<a><b>}\\Pi_{<a><r><a><r>}t_{<j>}^{J<a>}'
#            Note that this function can only be used for strings that
#            contain only one operator, a four-electron operator.
def strmatch4(str0, str1):
    #print "TESTING STRMATCH4"
    #First, make sure that the amplitudes match up perfectly.
    tia0_l = tia_re.findall(str0)
    gia0_l = gia_re.findall(str0)
    tia1_l = tia_re.findall(str1)
    gia1_l = gia_re.findall(str1)
    mystr0 = str(str0)
    mystr1 = str(str1)
    #print "mystr0 = " +mystr0
    #print "mystr1 = " +mystr1
    if (len(tia0_l)!=len(tia1_l)):
        return 0
    for i in range(len(tia0_l)):
        if (tia0_l[i]!=tia1_l[i]):
            return 0
        else:
            mystr0 = mystr0.replace(tia0_l[i],"")
            mystr1 = mystr1.replace(tia1_l[i],"")
    if (len(gia0_l)!=len(gia1_l)):
        return 0
    for i in range(len(gia0_l)):
        if (gia0_l[i]!=gia1_l[i]):
            return 0
        else:
            mystr0 = mystr0.replace(gia0_l[i],"")
            mystr1 = mystr1.replace(gia1_l[i],"")
    ind_l = tri_re.findall(mystr0)
    ind0 = ind_l[0]
    ind1 = ind_l[1]
    ind2 = ind_l[2]
    ind3 = ind_l[3]
    test0 = "\\Pi_{<"+ind0+"><"+ind1+"><"+ind3+"><"+ind2+">}"
    test1 = "\\Pi_{<"+ind3+"><"+ind1+"><"+ind0+"><"+ind2+">}"
    test2 = "\\Pi_{<"+ind0+"><"+ind2+"><"+ind3+"><"+ind1+">}"
    test3 = "\\Pi_{<"+ind3+"><"+ind2+"><"+ind0+"><"+ind1+">}"
    if (mystr1==test0 or mystr1==test1 or mystr1==test2 or mystr1==test3):
        return 1
    test0 = "\\Pi_{<"+ind1+"><"+ind0+"><"+ind2+"><"+ind3+">}"
    test1 = "\\Pi_{<"+ind2+"><"+ind0+"><"+ind1+"><"+ind3+">}"
    test2 = "\\Pi_{<"+ind1+"><"+ind3+"><"+ind2+"><"+ind0+">}"
    test3 = "\\Pi_{<"+ind2+"><"+ind3+"><"+ind1+"><"+ind0+">}"
    if (mystr1==test0 or mystr1==test1 or mystr1==test2 or mystr1==test3):
        return 1
    test0 = "\\Pi_{<"+ind0+"><"+ind3+"><"+ind1+"><"+ind2+">}"
    test1 = "\\Pi_{<"+ind1+"><"+ind3+"><"+ind0+"><"+ind2+">}"
    test2 = "\\Pi_{<"+ind0+"><"+ind2+"><"+ind1+"><"+ind3+">}"
    test3 = "\\Pi_{<"+ind1+"><"+ind2+"><"+ind0+"><"+ind3+">}"
    if (mystr1==test0 or mystr1==test1 or mystr1==test2 or mystr1==test3):
        return 2
    test0 = "\\Pi_{<"+ind3+"><"+ind0+"><"+ind2+"><"+ind1+">}"
    test1 = "\\Pi_{<"+ind3+"><"+ind1+"><"+ind2+"><"+ind0+">}"
    test2 = "\\Pi_{<"+ind2+"><"+ind0+"><"+ind3+"><"+ind1+">}"
    test3 = "\\Pi_{<"+ind2+"><"+ind1+"><"+ind3+"><"+ind0+">}"
    if (mystr1==test0 or mystr1==test1 or mystr1==test2 or mystr1==test3):
        return 2
    #print "STRMATCH4: Can't find any matches!"
    return 0

#Standardize output 4: If we're dealing with the two-electron
#operator part of the Hamiltonian (O2 = 1/2<rs|tu> = \Pi_{rstu}),
#write all operators as properly LaTeX-formatted versions of
#<rs|tu> or <rs||tu>=<rs|tu>-<rs|ut> and apply coefficient.
#WARNING: we assume real orbitals (no imaginary part!)
if (op_mode==2):
    for ti in range(len(out_lst)-1,-1,-1):
        matchfound = False
        tol = outterm([out_lst[ti]])
        temp_lst = re.findall(optind_re,(out_lst[ti])[2])
        temp_lst = temp_lst[0]
        temp_lst = re.findall(tri_re,temp_lst[0])
        ind0 = temp_lst[0]
        ind1 = temp_lst[1]
        ind2 = temp_lst[2]
        ind3 = temp_lst[3]
        s0 = "Pi_{<"+ind0+"><"+ind1+"><"+ind2+"><"+ind3+">}"
        #print "standardize output 4: tol :",tol
        for cti in range(ti-1,-1,-1):
            ctol = [out_lst[cti]]
            #print "ti = "+str(ti)+", cti = "+str(cti)+"."
            #print "coeff0 = "+str((tol[0])[0])+", coeff1 = "+str((ctol[0])[0])+"."
            #Check if we have a match of the form <rs|tu>-<rs|ut>,
            #convert it to <rs||tu> (=-<rs||ut>).
            #print "standardize output 4: ctol:",ctol
            mat4 = strmatch4((tol[0])[2],(ctol[0])[2])
            if ((tol[0])[0]==-(ctol[0])[0] and (tol[0])[1]==(ctol[0])[1] and mat4!=0):
                #print "standardize output 3: MATCH FOUND"
                #(out_lst[ti])[0] = -(out_lst[ti])[0]
                (out_lst[cti])[0] = 0
                if (ind2 != ind3 and mat4==1):
                    sf = "langle <"+ind0+"><"+ind1+">\\vert\\vert <"+ind2+"><"+ind3+">\\rangle "
                elif (ind2 != ind1 and mat4==2):
                    sf = "langle <"+ind0+"><"+ind3+">\\vert\\vert <"+ind2+"><"+ind1+">\\rangle "
                else:
                    #s1 = "langle <"+ind0+"><"+ind3+">\\vert\\vert <"+ind2+"><"+ind1+">\\rangle "
                    #If ind2=ind3, this should have been swept away (the two cancel each other out).
                    print "Error: cancelling terms not caught."
                    sys.exit()
                (out_lst[ti])[2] = (out_lst[ti])[2].replace(s0,sf)
                matchfound = True
                break
        if (not matchfound):
            #If there is no match, convert to non-antisymmetrized integral,
            #\Pi_{rstu} -> <rs|tu>
            s1 = "langle <"+ind0+"><"+ind1+">\\vert <"+ind2+"><"+ind3+">\\rangle "
            (out_lst[ti])[2] = (out_lst[ti])[2].replace(s0,s1)
        #Finally, always apply the 1/2 coefficient
        (out_lst[ti])[0] = 0.5*(out_lst[ti])[0]
out_lst = nozero(out_lst)
#printl("out_lst (4)",out_lst)

#Standardize output 5: Apply standard formatting to terms and
#indices.
for ti in range(len(out_lst)):
...
#    (out_lst[ti])[2] = re.sub(r"rangle",r"\rangle",(out_lst[ti])[2])
#    (out_lst[ti])[2] = re.sub(r"\\langle",r"\langle",(out_lst[ti])[2])
#    (out_lst[ti])[2] = re.sub(r"vert",r"\vert",(out_lst[ti])[2])
#printl("out_lst (5)",out_lst)
