...
for i in range(len(earr)):
    while len(earr[i])<max_cbp_e:
        earr[i] = sz + earr[i]
for i in range(len(Tarr)):
    while len(Tarr[i])<max_cbp_T:
        Tarr[i] = sz + Tarr[i]

if (len(Tarr[0])+len(earr[0])>6):
...
for Ts in Tarr:
    ...
    for es in earr:
        ...
        f_i.write(template_scrp_s.replace(mta,szta).replace(mtb,sztb).replace(minc,szinc).replace(meps,es).replace(mTmp,Ts).replace(mtia,stia))
        f_i.close()
    os.chdir("../")
template_scrp.close()
