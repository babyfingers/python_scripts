...
# v1.3: fixed bug that had to do with the length of the names of the
# output files.

# v1.2: fixed bug that had to do with converting integers with leading
# zeros into strings.

# v1.1: modified to deal with changes in the tia_order variable.
...
maxjobs = 100 #If we just do all the jobs at once, how many should we run at a time?
#Start and end values
#z (position) values
zta  = 2.6 #Start the z values where?
ztb  = 2.6 #End the z values where?
zinc = 0.2 #Increment by what value?
...
Ttb  = 20
Tinc = 1
#Type of overlap calculation to do (0 = none, 1 = light, 2 = full)
tia = 0
...
mtia = 'ttovlp' #Some marker for the type of overlap calc. to do
tmpfl = 'tempout.txt'
os.system("pwd > "+tmpfl)
f = open(tmpfl,'r')
fileloc = f.read()
...
sS  = 'S'
eols = "\n"
tm = 2.5 #Seconds between issuing qstat
...
if (len(str(earr[0]).zfill(max_cbp_e))+len(str(Tarr[0]).zfill(max_cbp_T))>6):
...
dirprfx = "T="
for Ts in Tarr:
    Tdir = dirprfx + str(Ts).zfill(max_cbp_T)
    os.system("mkdir "+Tdir)
    os.chdir(Tdir)
    for es in earr:
        inname_i = "T"+str(Ts).zfill(max_cbp_T)+"e"+str(es).zfill(max_cbp_e)+infilenameend
        Sinname_i = sS+"T"+str(Ts).zfill(max_cbp_T)+"e"+str(es).zfill(max_cbp_e)+infilenameend
        scriptname_i = scriptname_template+"T"+str(Ts).zfill(max_cbp_T)+"e"+str(es).zfill(max_cbp_e)+scriptnameend
        ...
        f_i.write(template_scrp_s.replace(mta,szta).replace(mtb,sztb).replace(minc,szinc).replace(meps,str(es)).replace(mTmp,str(Ts)).replace(mtia,stia))
        f_i.close()
    os.chdir("../")
template_scrp.close()
...
if Mmkrn==2:
    ...
    cmdname = "cmd.tmp"
    cmdend = "2\n1\n1\n1\n"
    qmk_r = re.compile("T[1-9][0-9]{0,6}e[1-9][0-9]{0,6}"+outfilenameend+".* [ERQ] batch") #What to look for in qstat to indicate a running job
    for Ts in Tarr:
        print "Working on T = "+str(Ts).zfill(max_cbp_T)
        Tdir = dirprfx + str(Ts).zfill(max_cbp_T)
        os.chdir(Tdir)
        for es in earr:
            print "Working on eps = " +str(es)
            dirname = "eps="+str(es).zfill(max_cbp_e)
            inname_i = "T"+str(Ts).zfill(max_cbp_T)+"e"+str(es).zfill(max_cbp_e)
            scriptname_i = scriptname_template+"T"+str(Ts).zfill(max_cbp_T)+"e"+str(es).zfill(max_cbp_e)+scriptnameend
            f_i = open(cmdname,'w')
