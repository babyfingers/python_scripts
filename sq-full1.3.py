...
#under the transformation rstu -> turs and under the
#transformation rstu -> tsru.
...
#operator part of the Hamiltonian (O2 = 1/2<rs|ut> = \Pi_{rstu}),
#write all operators as properly LaTeX-formatted versions of
#<rs|ut> or <rs||ut>=<rs|ut>-<rs|tu>m and apply coefficient.
...
if (op_mode==2):
    for ti in range(len(out_lst)-1,-1,-1):
        ...
        #Try to match a permutation
        s1 = "Pi_{<"+ind0+"><"+ind1+"><"+ind3+"><"+ind2+">}"
        (tol[0])[2] = (out_lst[ti])[2].replace(s0,s1)
        tol = opindord(tol)
        print "standardize output 4: tol :",tol
        for cti in range(ti-1,-1,-1):
            ctol = [out_lst[cti]]
            #Check if we have a match of the form <rs|tu>-<rs|ut>,
            #convert it to -<rs||tu> (=<rs||ut>). Note also that
            #the order of indices in the Pi operator changes when
            #we move to the Bra-Ket style operator.
            print "standardize output 4: ctol:",ctol
            if ((tol[0])[0]==-(ctol[0])[0] and (tol[0])[1]==(ctol[0])[1] and (tol[0])[2]==(ctol[0])[2]):
                print "standardize output 3: MATCH FOUND"
                (out_lst[ti])[0] = -(out_lst[ti])[0]
                (out_lst[cti])[0] = 0
                if (ind2 != ind3):
                    s1 = "langle <"+ind0+"><"+ind1+">\\vert\\vert <"+ind2+"><"+ind3+">\\rangle "
                else:
                ...
                break
        if (not matchfound):
            #If that didn't work, try a new permutation.
            s1 = "Pi_{<"+ind0+"><"+ind2+"><"+ind1+"><"+ind3+">}"
            (tol[0])[2] = (out_lst[ti])[2].replace(s0,s1)
            tol = opindord(tol)
            print "standardize output 4: tol :",tol
            for cti in range(ti-1,-1,-1):
        ...
        if (not matchfound):
            ...
            #\Pi_{rstu} -> <rs|ut>
            s1 = "langle <"+ind0+"><"+ind1+">\\vert <"+ind3+"><"+ind2+">\\rangle "
            (out_lst[ti])[2] = (out_lst[ti])[2].replace(s0,s1)
